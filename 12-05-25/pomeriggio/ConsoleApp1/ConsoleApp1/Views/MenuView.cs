    ...
    internal class MenuView
    {
        ...
        public void Start()
        {
            ...
            while (!exit)
            {
                ...
                Console.WriteLine("20) Attiva/disattiva logger globale");
                Console.WriteLine("21) Attiva/disattiva categorie");
                Console.WriteLine("22) Imposta output (file / database / entrambi)");
                Console.WriteLine("23) Imposta percorso file log");
                ...
                switch (Console.ReadLine())
                {
                    ...
                    case "20": LogGlob(); break;
                    case "21": LogCategoria(); break;
                    case "22": LogImpostaOutput(); break;
                    case "23": impostaPercorsoLog(); break;
                    default:
        ...
        private void LogGlob()
        {
            while (true)
            {
                Console.Write("Attivare logger globale? (s/n): ");
                string input = Console.ReadLine()?.Trim().ToLower();

                if (input == "s" || input == "n")
                {
                    bool attivo = input == "s";
                    LogService.Instance.AttivaDisattivaLoggerGlobale(attivo);
                    Console.WriteLine(attivo ? "Logger globale attivato." : "Logger globale disattivato.");
                    break;
                }
                else
                {
                    Console.WriteLine("Input non valido. Inserire 's' per sì o 'n' per no.");
                }
            }
        }

        private void LogCategoria()
        {
            string[] categorieValide = { "studenti", "professori", "corsi", "codestorico" };
            string cat;

            while (true)
            {
                Console.Write("Categoria (studenti/professori/corsi/codestorico): ");
                cat = Console.ReadLine()?.Trim().ToLower();

                if (categorieValide.Contains(cat))
                    break;

                Console.WriteLine("Categoria non valida. Riprovare.");
            }

            while (true)
            {
                Console.Write("Attivare la categoria? (s/n): ");
                string input = Console.ReadLine()?.Trim().ToLower();

                if (input == "s" || input == "n")
                {
                    bool att = input == "s";
                    LogService.Instance.ImpostaCategoria(cat, att);
                    Console.WriteLine(att
                        ? $"Log attivato per categoria {cat}."
                        : $"Log disattivato per categoria {cat}.");
                    break;
                }
                else
                {
        ...
        private void LogImpostaOutput()
        {
            bool file, db;

            while (true)
            {
                Console.Write("Usare file? (s/n): ");
                string inputFile = Console.ReadLine()?.Trim().ToLower();

                if (inputFile == "s" || inputFile == "n")
                {
                    file = inputFile == "s";
                    break;
                }
                ...
                {
                    Console.WriteLine("Input non valido. Inserire 's' o 'n'.");
                }
            }

            while (true)
            {
                Console.Write("Usare database? (s/n): ");
                string inputDb = Console.ReadLine()?.Trim().ToLower();

                if (inputDb == "s" || inputDb == "n")
                {
                    db = inputDb == "s"; 
                    break;
                }
            ...
            LogService.Instance.ImpostaOutput(file, db);
            Console.WriteLine($"Output log impostato: File={(file ? "Sì" : "No")}, Database={(db ? "Sì" : "No")}");
        }

        private void ImpostaPercorsoLog()
        {
            while (true)
            {
                ...
                {
                    Console.WriteLine("Percorso non valido. Riprovare.");
                    continue;
                }

                try
                {
                    LogService.Instance.ImpostaPath(path);
                    Console.WriteLine($"Percorso log aggiornato a: {path}");
                    break;
                }
                catch (UnauthorizedAccessException)
                {
                    Console.WriteLine("Accesso negato al percorso specificato. Scegliere un percorso con permessi di scrittura.");
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Errore nell'impostare il percorso: {ex.Message}");
                }
            }
        }
        ...
        private void AggiungiRichiestaCoda()
        {
            ...
            var corsi = controller.GetAllCorsi().ToList();

            Console.WriteLine("Corsi disponibili:");
            foreach (var corso in corsi)
                Console.WriteLine($"{corso.Codice}: {corso.Nome}");
            Console.Write("Codice corso: "); 
            string codCorso = Console.ReadLine().Trim().ToUpperInvariant();

            if (string.IsNullOrEmpty(codCorso) || !corsi.Exists(c => c.Codice == codCorso))
            {
                Console.WriteLine("Errore: devi selezionare un corso valido. Studente non aggiunto.");
                return;
            }
            controller.RichiestaNuovoStudente(n, c, m, codCorso);
            Console.WriteLine("Richiesta studente aggiunta in coda.");
        }
